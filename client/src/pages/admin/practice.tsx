...
export default function AdminPractice() {
    ...
    const { data: tests, isLoading } = useQuery<Test[]>({
        queryKey: ["/api/tests", { type: "practice" }],
        queryFn: async () => {
            const res = await apiRequest("GET", "/api/tests?type=practice");
            return res;
        }
    });
    ...
    const createTestMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            toast({
                title: editingTest ? "Session updated" : "Session created",
                description: editingTest ? "Practice session has been updated successfully" : "New practice session has been created",
            });
            setDialogOpen(false);
            setEditingTest(null);
    ...
    return (
        <div className="p-8 space-y-6 max-w-7xl mx-auto">
            <div className="flex items-center justify-between">
                ...
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    ...
                    <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                        <DialogHeader>
                            <DialogTitle>{editingTest ? "Edit Session" : "Add New Session"}</DialogTitle>
                            <DialogDescription>
                                {editingTest ? "Update session information" : "Create a new practice session"}
                            </DialogDescription>
                        </DialogHeader>
                        <Form {...form}>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input placeholder="e.g., JavaScript Basics Practice" {...field} data-testid="input-topic-name" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
            ...
            <div className="flex items-center gap-4">
                ...
                {searchQuery && (
                    <div className="text-sm text-muted-foreground">
                        {filteredTests.length} of {tests?.length || 0} sessions
                    </div>
                )}
            </div>
                ...
                tests && tests.length > 0 ? (
                    ...
                    <Card className="p-12">
                        <div className="text-center text-muted-foreground">
                            ...
                            <p className="text-sm">Click "Add Session" to create your first practice session</p>
                        </div>
                    </Card>
                )
            }
