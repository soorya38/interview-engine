...
echo -e "${GREEN}🚀 Starting FreelanceFlow Setup...${NC}"

# 1. Start Database via Docker (Port 5433 to avoid conflicts)
echo -e "${YELLOW}📦 Starting Database Container...${NC}"
if ! docker compose up -d postgres; then
    echo -e "${RED}❌ Failed to start Docker. Make sure Docker is running.${NC}"
    exit 1
fi
...
echo -e "${GREEN}👉 Access at: http://localhost:5173 or http://YOUR_PUBLIC_IP:5173${NC}"
